...
WORKER_COUNT=12
import random
os.chdir("/home/jerry/data/NMR/data_generation")
pynmrstar.ALLOW_V2_ENTRIES = True
WORKING_FOLDER="/home/jerry/data/NMR/datasets/new_splitting"
...
def build_spartap(seq_alignment_dict):
    for pdb_bmrb_id in seq_alignment_dict.keys():
        ...
        pdb_single_chain_files=[item for item in os.listdir(WORKING_FOLDER+"/chainsH") if pdbid.lower() in item]
        ...
        shifts,pH=get_shifts(WORKING_FOLDER+"/shifts/"+bmrbid+".tab")
        df=build_single_chain_df(WORKING_FOLDER+"/chainsH/"+pdb_single_chain_file,shifts,seq_alignment_dict[pdb_bmrb_id],pH=pH)
        df.to_csv(WORKING_FOLDER+"/datafiles/"+pdb_single_chain_file.split(".")[0]+".csv")

def build_shiftx2(pdb_to_shift_dict):
    for pdbid in pdb_to_shift_dict:
        pid=pdbid[:4]
        print("Processing SHIFTX2 structure: "+pid)
        pdb_single_chain_files=[item for item in os.listdir(WORKING_FOLDER+"/chainsH") if pid in item]
        if len(pdb_single_chain_files)!=1:
            print("Ignored %s"%pid)
            continue
        else:
        ...
        shifts,pH=get_shifts(WORKING_FOLDER+"/shifts/"+shift_file)
        df=build_single_chain_df(WORKING_FOLDER+"/chainsH/"+pdb_single_chain_file,shifts,pH=pH)
        df.to_csv(WORKING_FOLDER+"/datafiles/"+pdb_single_chain_file.split(".")[0]+".csv")
...
def build_refdb_test(pdb_bmr_dict):
    for pdbid in pdb_bmr_dict:
        ...
        if os.path.exists(file_pos):
            shift_pos="/home/jerry/data/NMR/refDB/bmr%d.str.corr"%pdb_bmr_dict[pdbid]
